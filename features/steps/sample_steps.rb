...
And(/^I can tap on image "([^"]*)"$/) do |file_name|
  touch_image(file_name)
end

And(/^I should see image "([^"]*)"$/) do |template_file|
  fail 'Cannot find image' unless should_see_image(template_file)
end